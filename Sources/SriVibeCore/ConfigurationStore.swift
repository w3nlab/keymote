...
public final class ConfigurationStore {
    ...
    private let key = "SriVibe.configuration.v1"

    public init(defaults: UserDefaults = .standard) { self.defaults = defaults }

    public func load() -> AppConfiguration {
        guard let data = defaults.data(forKey: key), let value = try? JSONDecoder().decode(AppConfiguration.self, from: data) else {
        ...
        for profile in AppProfile.allCases {
            ...
            for button in [RemoteButton.volumeUp, .volumeDown] where !mapping.bindings.contains(where: { $0.button == button }) {
