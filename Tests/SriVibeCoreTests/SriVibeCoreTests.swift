...
@Test func profileResolutionAndDefaultMappings() {
    ...
    #expect(config.mappings[.chatGPT]?.action(for: .tv, gesture: .hold) == .switchApplication)
    #expect(config.mappings[.terminal]?.action(for: .playPause, gesture: .tap) == .nextTerminalTab)
    #expect(config.mappings[.terminal]?.action(for: .playPause, gesture: .hold) == .previousTerminalTab)
    #expect(config.mappings[.otty]?.action(for: .playPause, gesture: .tap) == .nextTerminalTab)
    #expect(config.mappings[.chrome]?.action(for: .up, gesture: .tap) == .arrowUp)
    #expect(config.mappings[.chrome]?.action(for: .playPause, gesture: .hold) == .previousTerminalTab)
}
